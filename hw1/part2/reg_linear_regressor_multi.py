...
class RegularizedLinearReg_SquaredLoss(RegularizedLinearRegressor_Multi):
    ...
    def loss (self,*args):
        ...
        J = (np.dot(np.transpose(temp), temp) + reg * theta[1:].dot(np.transpose(theta[1:]))) / (2. * len(X))
    ...
    def grad_loss(self,*args):                                                                          
        ...
        temp = np.dot(X, theta) - y
        grad[0] = np.transpose(X[:, 0]).dot(temp) / float(len(X))
        grad[1:] = (np.transpose(X[:, 1:]).dot(temp) + reg * theta[1:]) / float(len(X))
