...
def learning_curve(X,y,Xval,yval,reg):
    ...
    for i in range(2, num_examples+1):
        ...
        theta_opt = reglinear_reg.train(X[:i, :], y[:i], reg=reg, num_iters=1000)
        error_train[i-1] = reglinear_reg.loss(theta_opt, X[:i, :], y[:i], 0)
        error_val[i-1] = reglinear_reg.loss(theta_opt, Xval, yval, 0)
...
def averaged_learning_curve(X,y,Xval,yval,reg):
    ...
    for i in range(2, num_examples+1):
        for j in range(50):
            sample = random.sample(set(range(0, num_examples)), i)
            reglinear_reg = RegularizedLinearReg_SquaredLoss()
            theta_opt = reglinear_reg.train(X[sample, :], y[sample], reg=reg, num_iters=1000)
            error_train[i-1] += (reglinear_reg.loss(theta_opt, X[sample, :], y[sample], 0) / 50.)
            error_val[i-1] += (reglinear_reg.loss(theta_opt, Xval[sample, :], yval[sample], 0) / 50.)
